"""Frozen stage probes and teacher-forced target metrics."""
...
from posttrain_circuits.datasets.proofgraph.generation import GENERATOR_VERSION, LABEL_SEMANTICS
from posttrain_circuits.datasets.proofgraph.rendering import render_proof_literal, render_step
...
class CircuitProbeSpec:
    ...
    @classmethod
    def from_mapping(cls, row: dict[str, Any]) -> CircuitProbeSpec:
        payload = dict(row)
        for field_name in (
            "source_example_ids",
            "clean_input_ids",
        ...
        payload["alignment_checks"] = tuple(
            (str(name), bool(passed)) for name, passed in payload["alignment_checks"]
        )
        return cls(**payload)
    ...
    def __post_init__(self) -> None:
        ...
        if not self.clean_intervention_positions or not self.corrupt_intervention_positions:
            raise ValueError("circuit interventions require explicit nonempty token positions")
        if len(self.clean_intervention_positions) != len(self.corrupt_intervention_positions):
            raise ValueError("clean/corrupt intervention positions must be paired one-to-one")
        for side, positions, input_ids in (
            ("clean", self.clean_intervention_positions, self.clean_input_ids),
            ("corrupt", self.corrupt_intervention_positions, self.corrupt_input_ids),
        ):
            if positions != tuple(sorted(set(positions))):
                raise ValueError(f"{side} intervention positions must be sorted and unique")
            if positions[0] < 0 or positions[-1] >= len(input_ids):
                raise ValueError(f"{side} intervention position is outside model input")
        if any(
            self.clean_input_ids[clean_position] == self.corrupt_input_ids[corrupt_position]
            ...
            raise ValueError("explicit intervention positions must identify changed token pairs")
...
def _teacher_forced_encoding(
    ...
    context: str,
    target: str,
) -> tuple[tuple[int, ...], tuple[int, ...], tuple[int, ...], int, str]:
    ...
    # A tokenizer may form the first target token from a suffix of the context
    # (commonly a trailing space) and the first target characters.  That token
    # is the causal boundary target and must be scored, not discarded.
    target_positions = tuple(index for index, (_start, end) in enumerate(offsets) if end > boundary)
    ...
    if target_start == 0:
        raise ValueError("target boundary has no preceding token for causal scoring")
    first_start, first_end = offsets[target_start]
    if not first_start <= boundary < first_end:
        raise ValueError("first target token does not cover the context/target boundary")
    ...
    return model_input_ids, target_ids, metric_positions, target_start, model_input_text
...
def tokenize_probe_specs(
    ...
    *,
    ...
    for row in semantic_manifest["probes"]:
        ...
        clean_ids, clean_targets, clean_positions, clean_target_start, clean_text = _teacher_forced_encoding(
            tokenizer, clean_context, semantic.clean_target
        )
        corrupt_ids, corrupt_targets, corrupt_positions, corrupt_target_start, corrupt_text = (
            _teacher_forced_encoding(
        ...
        shared_context_length = min(clean_target_start, corrupt_target_start)
        intervention_positions = tuple(
            index
            for index, (clean_id, corrupt_id) in enumerate(
                zip(clean_ids[:shared_context_length], corrupt_ids[:shared_context_length], strict=False)
            )
        ...
        checks = (
            ...
            ("target_boundary_positions_aligned", clean_target_start == corrupt_target_start),
            ("first_target_boundary_token_included", clean_positions[0] == clean_target_start - 1),
            ("explicit_intervention_positions_nonempty", bool(intervention_positions)),
        )
        base = {
            ...
            "clean_intervention_positions": intervention_positions,
            "corrupt_intervention_positions": intervention_positions,
