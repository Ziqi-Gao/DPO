"""Execute pinned MIB attribution in the foreground on fixed pairs."""
...
class _FixedPairDataset(Dataset):
    def __init__(
        ...
        for index in indices:
            ...
            clean_intervention_positions = [int(value) for value in row["clean_intervention_positions"]]
            corrupt_intervention_positions = [
                int(value) for value in row["corrupt_intervention_positions"]
            ]
            if not clean_intervention_positions or len(clean_intervention_positions) != len(
                corrupt_intervention_positions
            ):
                raise ValueError(f"pair {row['pair_id']} intervention positions are invalid")
            if any(
                clean_tokens[clean_position] == corrupt_tokens[corrupt_position]
                ...
                raise ValueError(f"pair {row['pair_id']} intervention positions include unchanged tokens")
            self.items.append(
                (
                    row["clean_prompt"],
                    row["corrupt_prompt"],
                    {
                        ...
                        "clean_intervention_positions": clean_intervention_positions,
                        "corrupt_intervention_positions": corrupt_intervention_positions,
...
def main(argv: list[str] | None = None) -> None:
    ...
    for component in primary_scores:
        ...
        uncertainty[component] = statistics.stdev(values)
    atomic_write_json(
        args.output,
        {
            "backend": "mib-eap-ig",
