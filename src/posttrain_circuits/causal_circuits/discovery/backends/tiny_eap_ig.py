"""Activation-space EAP-IG backend for deterministic tiny CPU smoke."""
...
from posttrain_circuits.causal_circuits.contracts import CircuitScores
from posttrain_circuits.causal_circuits.metrics.probes import TargetSequenceMetric
...
class TinyEapIgBackend:
    version = "tiny-hf-eap-ig-activations-v2-explicit-positions"
    ...
    def _pair_scores(
        ...
        for name in names:
            clean_index = torch.tensor(
                ...
                dtype=torch.long,
                device=clean_cache[name].device,
            )
            corrupt_index = torch.tensor(
                pair.corrupt_intervention_positions,
                dtype=torch.long,
                device=corrupt_cache[name].device,
            )
            clean = clean_cache[name].index_select(-2, clean_index)
            corrupt = corrupt_cache[name].index_select(-2, corrupt_index)
            delta = clean - corrupt
            gradients = []
            for step in range(
                1,
                ...
                logits = exact._run_patched(
                    ...
                    replacement_positions=tuple(range(len(pair.clean_intervention_positions))),
                )
    ...
    def score_all_components(
        ...
        for name in names:
            ...
            uncertainty[name] = statistics.stdev(values)
