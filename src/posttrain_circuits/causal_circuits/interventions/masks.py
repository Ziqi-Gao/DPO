"""Score-threshold and strictly layer-matched random masks."""
...
from posttrain_circuits.causal_circuits.contracts import CircuitMask
...
def layer_matched_random_mask(universe: tuple[str, ...], reference: CircuitMask, seed: int) -> CircuitMask:
    ...
    if not reference_components <= set(universe):
        raise ValueError("reference mask contains components outside the control universe")
    ...
    for layer, count in needed.items():
        choices = [component for component in by_layer[layer] if component not in reference_components]
        rng.shuffle(choices)
        if len(choices) < count:
            raise ValueError(f"no sufficient non-circuit random controls in layer {layer}")
    ...
    *,
    ...
    for repeat in range(repeats):
        ...
        for component in references:
            ...
            if not same_layer:
                raise ValueError(
                    f"no non-circuit activation-matched control in layer {_layer(component)}"
                )
            ...
            choice = min(same_layer, key=distance)
