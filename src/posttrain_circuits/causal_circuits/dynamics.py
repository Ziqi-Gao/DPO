"""Time-resolved continuous and thresholded circuit dynamics."""
...
def _aligned(
    ...
    return (
        tuple(float(a.get(key, 0.0)) for key in keys),
        tuple(float(b.get(key, 0.0)) for key in keys),
    )
...
def weighted_overlap(
    ...
    av, bv = _aligned(a, b)
    denominator = sum(max(abs(left), abs(right)) for left, right in zip(av, bv, strict=True))
    numerator = sum(min(abs(left), abs(right)) for left, right in zip(av, bv, strict=True))
...
def attribution_rank_stability(
    ...
    av, bv = _aligned(a, b)
    if len(av) < 2:
        raise ValueError("Spearman stability requires at least two aligned components")
    if not all(math.isfinite(value) for value in (*av, *bv)):
        raise ValueError("Spearman stability requires finite score vectors")

    def average_ranks(values: tuple[float, ...]) -> list[float]:
    ...
    br_mean = sum(br) / len(br)
    ar = [value - ar_mean for value in ar]
    br = [value - br_mean for value in br]
    denominator = math.sqrt(sum(value * value for value in ar)) * math.sqrt(
        sum(value * value for value in br)
    )
    if denominator == 0.0:
        raise ValueError("Spearman stability is undefined for a constant score vector")
    correlation = sum(left * right for left, right in zip(ar, br, strict=True)) / denominator
...
def continuous_churn(
    ...
    av, bv = _aligned(a, b)
    denominator = sum(abs(value) for value in av) + sum(abs(value) for value in bv)
    numerator = sum(abs(left - right) for left, right in zip(av, bv, strict=True))
...
def estimate_estimator_noise_floor(
    ...
    *,
    ...
    return {
        ...
        "estimated_noise_churn": statistics.fmean(continuous),
        "estimated_thresholded_mask_churn": statistics.fmean(thresholded),
        "estimated_thresholded_jaccard": 1.0 - statistics.fmean(thresholded),
        "within_checkpoint_full_score_spearman": statistics.fmean(spearman),
        "within_checkpoint_weighted_overlap": statistics.fmean(overlaps),
    }
...
def locking_time_bootstrap(
    ...
    *,
    ...
    return {
        "estimate": float(statistics.median(locking_steps)),
        "lower": quantile(locking_steps, 0.025),
        "upper": quantile(locking_steps, 0.975),
        "confidence": 0.95,
