...
FIXED_BANK_CURSOR_PROTOCOL_ID = "fixed-bank-round-robin-v1"
...
class FixedBankStateSource:
    ...
    def state_dict(self) -> dict[str, Any]:
        return {
            "kind": "fixed_bank",
            "cursor_protocol_id": FIXED_BANK_CURSOR_PROTOCOL_ID,
            "cursor": dict(self._cursor),
            "immutable_fingerprint": list(self.immutable_fingerprint),
        }

    def load_state_dict(self, state: dict[str, Any]) -> None:
        if state.get("cursor_protocol_id") != FIXED_BANK_CURSOR_PROTOCOL_ID:
            raise ValueError("fixed-bank checkpoint changed cursor protocol")
