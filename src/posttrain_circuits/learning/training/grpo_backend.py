...
from posttrain_circuits.methods.rl import TRL_GRPO_BATCH_CONTRACT, TRL_GRPO_VERSION
from posttrain_circuits.learning.training.token_budget import TOKEN_BUDGET_UNIT, distributed_token_sum
...
def require_pinned_trl_version() -> str:
    try:
        observed = importlib.metadata.version("trl")
    except importlib.metadata.PackageNotFoundError as error:
        raise RuntimeError("canonical GRPO requires the pinned TRL package") from error
    if observed != TRL_GRPO_VERSION:
        raise RuntimeError(
            f"canonical GRPO requires trl=={TRL_GRPO_VERSION}, observed={observed}"
        )
    return observed
...
def resolve_grpo_batch_contract(settings: GrpoSettings, *, world_size: int) -> dict[str, int | str]:
    ...
    return {
        "trl_version_contract": TRL_GRPO_BATCH_CONTRACT,
